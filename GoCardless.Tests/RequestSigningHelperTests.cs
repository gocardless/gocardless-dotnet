...
namespace GoCardless.Tests
{
  public class RequestSigningHelperTests
  {
    ...
    const string SIG_INPUT_NO_CONTENT = @"sig-1=(""@method"" ""@authority"" ""@request-target"");keyid=""PublicKeyId"";created=123;nonce=""nonce""";
    const string SIG_INPUT_WITH_CONTENT = @"sig-1=(""@method"" ""@authority"" ""@request-target"" ""content-digest"" ""content-type"" ""content-length"");keyid=""PublicKeyId"";created=123;nonce=""nonce""";
    ...
    [TestCase("test content", SIG_INPUT_WITH_CONTENT, "sha256=:auinVVUgn9bEQVfArtgBbnY/9DWhnPGG92hjFAFD/3I=:")]
    public async Task ShouldSignRequest(string content, string expectedSigInput, string expectedContentDigestHeader)
    {
      ...
      RequestSigningHelper.SignRequest(request, requestSigningSettings, content, _testMode: true);
      ...
      var contentDigest = haveDigest ? request.Headers.GetValues("Content-Digest").FirstOrDefault() : null;
