...
namespace GoCardless.Tests
{
    public class BlocksBlockByRefCodeSampleTest
    {
        ...
        [Test]
        public async Task BlockByRefCodeSampleExecutesWithoutError()
        {
            // Mock response - enqueue multiple times to handle code samples with multiple API calls
            string responseBody =
                "{ \"blocks\": [{}], \"meta\": { \"cursors\": {}, \"limit\": 50 } }";
            for (int i = 0; i < 5; i++)
            {
            ...
            try
            {
                var resp = await client.Blocks.BlockByRefAsync(
                    new GoCardless.Services.BlockBlockByRefRequest()
                    {
