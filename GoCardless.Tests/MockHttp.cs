...
namespace GoCardless.Tests
{
    ...
    {
        private readonly Queue<Tuple<HttpResponseMessage, Action<HttpResponseMessage>>> _queuedMessages = new Queue<Tuple<HttpResponseMessage, Action<HttpResponseMessage>>>();
        private readonly Queue<Tuple<HttpRequestMessage, string>> _requests = new Queue<Tuple<HttpRequestMessage, string>>();

        public void EnqueueResponse(int statusCode, string pathToBodyDocument, Action<HttpResponseMessage> transform = null)
        {
            var httpResponseMessage = new HttpResponseMessage((HttpStatusCode) statusCode)
            {
                Content = new StringContent(File.ReadAllText(pathToBodyDocument), Encoding.UTF8, "application/json")
            };
        ...
        {
            string content = request.Content is StringContent ? (await request.Content.ReadAsStringAsync()) : null;
