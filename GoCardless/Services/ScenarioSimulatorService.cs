...
namespace GoCardless.Services
{
    ...
    public class ScenarioSimulatorService
    {
        ...
        /// simulations. One of:
        ///
        /// - `creditor_verification_status_action_required`: Sets a creditor's `verification
        /// status` to `action required`, meaning that the creditor must provide further information
        /// to GoCardless in order to verify their account to receive payouts.
        /// - `creditor_verification_status_in_review`: Sets a creditor's `verification status` to
        ...
        /// - `payment_confirmed`: Transitions a payment through to `confirmed`. It must start in
        ...
        /// successfully and paid out to you. It must start in the `pending_submission` state, and
        /// its mandate must be in the `activated` state (unless it is a payment for ACH, BECS,
        /// BECS_NZ or SEPA, in which cases the mandate may be `pending_submission`, since their
        ...
        /// `pending_submission`, since their mandates are submitted with their first payment).
        /// - `payment_charged_back`: Behaves the same as the `payout_paid_out` simulator, except
        /// that the payment is transitioned to `charged_back` after it is paid out, having been
        /// charged back by the customer.
        /// - `payment_chargeback_settled`: Behaves the same as the `payment_charged_back`
        /// simulator, except that the charged back payment is additionally included as a debit item
        /// in a payout, thereby settling the charged back payment.
        ...
        /// - `payment_late_failure_settled`: Behaves the same as the `payment_late_failure`
        ...
        /// - `mandate_activated`: Transitions a mandate through to `activated`, having been
        ...
        /// which are submitted and activated with their first payment.
        ...
        /// the mandate will become `active`.
        ...
        /// subscriptions associated with the mandate will become `active`.
        /// - `mandate_failed`: Transitions a mandate through to `failed`, having been submitted to
        /// the banks but found to be invalid (for example due to invalid bank details). It must
        /// start in the `pending_submission` or `submitted` states. Not compatible with SEPA
        /// mandates, which are submitted with their first payment.
        /// - `mandate_expired`: Transitions a mandate through to `expired`, having been submitted
        ...
        /// mandates.
        /// - `mandate_transferred_with_resubmission`: Transitions a mandate through `transferred`
        ...
        /// must start in the `pending_submission` state. Only compatible with Bacs mandates.
        /// - `mandate_suspended_by_payer`: Transitions a mandate to `suspended_by_payer`, as if
        /// payer has suspended the mandate after it has been setup successfully. It must start in
        /// the `activated` state. Only compatible with PAY_TO mandates.
        /// - `refund_paid`: Transitions a refund to `paid`. It must start in either the
        /// `pending_submission` or `submitted` state.
        /// - `refund_settled`: Transitions a refund to `paid`, if it's not already, then generates
        /// a payout that includes the refund, thereby settling the funds. It must start in one of
        ...
        /// - `billing_request_fulfilled_and_payment_failed`: Authorises the billing request,
        ...
        /// billing requests with a `payment_request` are supported.
        ...
        /// supported.
        /// - `billing_request_fulfilled_and_payment_paid_out`: Authorises the billing request,
        ...
        /// billing requests with a `payment_request` are supported.</param>
