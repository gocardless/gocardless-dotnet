...
namespace GoCardless.Services
{
    ...
    ///
    ///  Scenario Simulators allow you to manually trigger and test certain
    ///  paths that your
    ...
    public class ScenarioSimulatorService
    {
        ...
        ///  status` to `action required`, meaning that the creditor must provide further
        ///  information to GoCardless in order to verify their account to receive payouts.</li>
        ///  <li>`creditor_verification_status_in_review`: Sets a creditor's `verification status`
        ///  to `in review`, meaning that the creditor has provided all of the information requested
        ...
        ///  the `pending_submission` state, and its mandate must be in the `activated` state
        ...
        ///  state, and its mandate must be in the `activated` state (unless it is a payment for
        ///  ACH, BECS, BECS_NZ or SEPA, in which cases the mandate may be `pending_submission`,
        ...
        ///  simulator, except that the charged back payment is additionally included as a debit
        ///  item in a payout, thereby settling the charged back payment.</li>
        ///  <li>`payment_late_failure`: Transitions a payment through to `late_failure`, having
        ///  been apparently collected successfully beforehand. It must start in either the
        ///  `pending_submission` or `paid_out` state, and its mandate must be in the `activated`
        ///  state (unless it is a payment for ACH, BECS, BECS_NZ or SEPA, in which cases the
        ///  mandate may be `pending_submission`, since their mandates are submitted with their
        ...
        ///  `pending_submission`, as if the customer approved the mandate creation. It must start
        ///  in the `pending_customer_approval` state. Compatible only with Bacs and SEPA mandates,
        ...
        ///  <li>`mandate_expired`: Transitions a mandate through to `expired`, having been
        ...
        ///  start in the `pending_submission` state. Not compatible with Autogiro, BECS NZ, and PAD
        ///  mandates, which do not expire.</li>
        ///  <li>`mandate_transferred`: Transitions a mandate through to `transferred`, having been
        ///  submitted to the banks, set up successfully and then moved to a new bank account due.
        ///  It must start in the `pending_submission` state. Only compatible with Bacs and SEPA
        ///  mandates.</li>
        ///  <li>`mandate_transferred_with_resubmission`: Transitions a mandate through
        ///  `transferred` and resubmits it to the banks, can be caused be the UK's Current Account
        ///  Switching Service (CASS) or when a customer contacts GoCardless to change their bank
        ///  details. It must start in the `pending_submission` state. Only compatible with Bacs
        ///  mandates.</li>
        ...
        ///  fulfils it, and moves the associated payment to `failed`. The billing request must be
        ///  in the `pending` state, with all actions completed except for `bank_authorisation`.
        ///  Only billing requests with a `payment_request` are supported.</li>
        ...
        ///  `failed`. The billing request must be in the `pending` state, with all actions
